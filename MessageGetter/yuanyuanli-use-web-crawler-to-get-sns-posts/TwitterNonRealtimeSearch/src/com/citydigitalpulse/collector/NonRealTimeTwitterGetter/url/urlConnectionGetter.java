...
/**
 * send GET request using urlConnection and place id
 * 
 * @author yuanyuan
 */
public class urlConnectionGetter {
	...
	public void Getter(queuing queInfor) {
		this.que = queInfor;
		this.place_id = que.get_place_id();
		 ...
		 * que.get_in_what_lan(); }
		 */
		String url = "https://twitter.com/search?f=tweets&vertical=default&q=place%3A" + place_id + "%20since%3A"
				+ startDate + "%20until%3A" + endDate;
		/*
		 * String url =
	...
	public void FirstTimeGetter(String urlID, queuing queInfor) {
		...
		try {
			...
			System.out.println("Crawling url is " + url);
			...
			if (reponse.equals("null")) {
				...
				if (has_more_items) {
					...
					System.out.println("Get Request sent succesfully " + last_note_ts);
	...
	public void PostGet(String minID, String maxID, String urlid, queuing queInfor) {
		...
		while (flag == true) {
			try {
				...
				System.out.println("Second Crawling Url is " + url);
				...
				if (response.equals("null")) {
					...
					if (has_more_items) {
						htmlParser.Parsing();
						MinTweetID = htmlParser.ReturnMinId();
						System.out.println("MinTweetID is" + MinTweetID);
						System.out.println("lastMinTWeetID is " + lastMinTWeetID);
					}
