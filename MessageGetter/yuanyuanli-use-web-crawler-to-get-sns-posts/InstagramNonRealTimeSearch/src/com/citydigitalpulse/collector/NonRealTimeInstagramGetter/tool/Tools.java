...
import com.citydigitalpulse.collector.NonRealTimeInstagramGetter.tool.LanguageFile;
...
public class Tools {
	...
	private static List<DetectedLanguage> lang;
	private static LanguageDetector languageDetector = LanguageFile.GetLanguageFile();
	public static ArrayList<MessageModel> msgList = new ArrayList<MessageModel>();
	public static HashMap<String, MessageModel> cacheUpdateMessages = new HashMap<String, MessageModel>();
	...
	public static void sendNewMessage(String url, String token, MessageModel msg) {
		...
		postMethod.addParameter("profile_image_url", msg.getProfile_img());
	...
	private static final int secondsOfaDay = 24 * 3600 * 1000;
	...
	public static String getYesterday(String currentDate) throws ParseException {
		DateFormat dateFormat = new SimpleDateFormat(CURRENT_DATE_FORMAT);
		Date date = dateFormat.parse(currentDate);
		return format(new Date(date.getTime() - secondsOfaDay));
	}

	public static String getTomorrow(String currentDate) throws ParseException {
		DateFormat dateFormat = new SimpleDateFormat(CURRENT_DATE_FORMAT);
		Date date = dateFormat.parse(currentDate);
		return format(new Date(date.getTime() + secondsOfaDay));
	}

	public static String langugaeDetection(String Text) throws IOException {
		// query:
		lang = languageDetector.getProbabilities(Text);
		String lnguage = "";
		if (lang != null && !lang.isEmpty()) {
			if (lang.get(0).getLocale().toString().equals("zh-CN")
					|| lang.get(0).getLocale().toString().equals("zh-TW")) {
				lnguage = "zh";
			} else {
				lnguage = lang.get(0).getLocale().toString();
			}
		}
		return lnguage;
	}
