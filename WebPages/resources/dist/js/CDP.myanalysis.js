        ...
        //get rank for user regions from the server
        function getUserRegionsRanking(date) {
            if (user_id != null && logintoken != null) {
                ...
                $.ajax({
                    type: "GET",
                    crossDomain: true,
                    url: serverURL + "message/getuserregionranks",
                    data: {
                        userID: user_id,
                        token: logintoken,
                        ...
                        if (data.code == 404) {
                            ...
                            if (data.obj != null) {
                                ...
                                for (var i = 0; i < numTables; i++) {
                                    tableHTMLArrray[i] = "<div class='item'><div class='table-responsive'><table class='table no-margin cityRanking'><thead><tr><th width='40'>Select</th><th width='40'>Rank</th><th>City</th><th width='50'>Messages</th><th width='50'>Value</th></tr></thead><tbody>";
                                }
                                var rank = 1;
                                $.each(cityRankingData, function () {
                                    if ((this.pulse.sum_num - this.pulse.unknown_num) >= 500) {
                                        tableHTMLArrray[Math.floor((rank - 1) / numPerTable)] += "<tr placeID='" + this.regID + "' recordID='" + this.record_id + "'><td><input type='checkbox' class='minimal'></td><td>" + rank + "</td><td><a class='cityName'>" + this.regName.toUpperCase() + "</a></td><td><span class='label label-warning'>" + this.pulse.sum_num + "</span></td><td><span class='label label-success'>" + this.pulse.pulse_value.toFixed(4) + "</span></td></tr>";
                                    } else {
                                        tableHTMLArrray[Math.floor((rank - 1) / numPerTable)] += "<tr placeID='" + this.regID + "' recordID='" + this.record_id + "'><td><input type='checkbox' class='minimal multiplecity'></td><td>" + rank + "</td><td><a class='cityName'>" + this.regName.toUpperCase() + "</a></td><td><span class='label label-warning'>" + this.pulse.sum_num + "</span></td><td>undefined</td></tr>";
                                    }
                                    rank++;
                                });
                                ...
                                owl.owlCarousel({
                                    ...
                                    itemsDesktop: [1200, 1], //2 items between 1200 and 992
                                    itemsDesktopSmall: [970, 1], // 1 item betweem 900px and 400
                                    itemsTablet: [710, 1], //1 item between 400 and 0
        ...
        $(function () {
            ...
            //    getUserRegionsRanking(cityRankDate);
            getUserRegionsRanking(today.addDays(-1));
