        ...
        // function to get total annotation number and display it in [Recently Annotated Messages] box
        ...
        // function to get recent annotation records for [Recently Annotated Messages] box
        function getrecentannotation() {
            if (user_id != null && logintoken != null) {
                $.ajax({
                    ...
                    success: function (data, textStatus) {
                        if (data.code == 200) {
                            //operations when getting recent record successfully
                            $("#recent-annotation").empty();
                            $.each(data.obj, function () {
                                $("#recent-annotation").append("<li class='item'><div class='product-img'><img src='" + this.media_urls[0] + "' alt='Product Image'></div><div class='product-info'><a href='javascript::;' class='product-title'>" + this.mark_at + "<span class='label pull-right label-recent'>" + this.emotion_text + "</span></a><span class='product-description'>" + this.text + "</span></div></li>");
                            });
        ...
        function getnewmessage() {
            ...
            annotation_part = $('input[type="radio"][name="annotation-part"]:checked').val();
            if (user_id != null && logintoken != null) {
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus) {
                        if (data.code == 200) {
                            ...
                            $.each(data.obj.medias, function () {
                                $("#owl-demo").append("<div><img class='col-md-12' src='" + this.media_url + "' /><center><div class='btn-group img-btn-group'  data-toggle='buttons'><label class='btn btn-app btn-emo btn-positive'><input type='radio' name='options' id='positive'><i class='fa fa-smile-o'></i>Positive</label><label class='btn btn-app btn-emo btn-neutral'><input type='radio' name='options' id='neutral'><i class='fa fa-meh-o'></i>Neutral</label><label class='btn btn-app btn-emo btn-negative'><input type='radio' name='options' id='negative'><i class='fa fa-frown-o'></i>Negative</label></div></center></div>");
                            });
